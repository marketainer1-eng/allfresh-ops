...
// app/demo/performance/page.tsx
// 공개 데모(인증/DB 불필요). /app/performance 의 클라이언트 UX를 그대로 시연한다.
// 차이점: 해석 요청을 공개 엔드포인트(/api/demo/analyze-performance)로 보낸다.
// raw CSV는 브라우저에서만 파싱되고, 서버에는 계산된 metrics(PerfRequest)만 전송된다.
...
export default function DemoPerformanceAnalysisPage() {
  ...
  async function handleAnalyze() {
    ...
    setError(null);
    try {
      const res = await fetch("/api/demo/analyze-performance", {
        method: "POST",
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      <div className="max-w-5xl mx-auto space-y-6">
        {/* Demo banner */}
        <div className="flex items-start gap-2 bg-blue-50 border border-blue-100 text-blue-800 rounded-lg p-3 text-sm">
          <Info className="w-4 h-4 mt-0.5 flex-shrink-0" />
          <span>
            공개 시연 모드입니다(로그인·DB 불필요). 업로드한 파일은 브라우저에서만 파싱되며, 서버에는
            계산된 지표만 전송됩니다. AI 해석은 키 미설정 시 규칙기반으로 동작합니다.
          </span>
        </div>
