...
//   LLM에게 raw CSV로 수치 계산을 시키지 않는다(수치 날조 방지).
// 방법론 출처: allfresh-integrated-analysis 스킬(SKILL.md / category-lens-fruit-gift / cross-channel-integration)을 시스템 프롬프트로 증류.
// 원본(Deno 엣지함수) → Next.js Route Handler 이식. 입력/출력 계약과 METHODOLOGY는 원본을 보존.
...
// ───────────────────────── 방법론(시스템 프롬프트) ─────────────────────────
// allfresh-integrated-analysis 스킬의 핵심 규칙을 증류.
